...
class RedGymEnv(Env):
    ...
    #     # Curiosity-driven component
    #     x, y, map_n = self.get_game_coords()
    #     position_key = f"x:{x} y:{y} m:{map_n}"  # Unique identifier for the position

    #     # Reward for new positions, penalty for revisits
    #     if position_key not in self.seen_coords:
    #         curiosity_reward = self.explore_weight * 0.2  # Reward for visiting a new position
    #         self.seen_coords[position_key] = 1  # Mark as visited in seen_coords
    #     else:
    #         curiosity_reward = -0.05  # Penalty for revisiting old position

    #     # Add curiosity reward to the "explore" category
    #     state_scores["explore"] += curiosity_reward

    #     return state_scores

    # def update_reward(self):
    #     # Compute the updated reward using the enhanced game state reward
    #     self.progress_reward = self.get_game_state_reward()

    #     # Sum all reward components for the current step
    #     new_total = sum([val for _, val in self.progress_reward.items()])
    #     new_step = new_total - self.total_reward  # Incremental reward for the step

    #     # Update the total reward to include the current step's progress
    #     self.total_reward = new_total
    ...
    #         "event": self.reward_scale * self.update_max_event_rew() * 4,
    #         "level": self.reward_scale * self.get_levels_reward(),
    #         "heal": self.reward_scale * self.total_healing_rew * 15, #changed from 30 to 15
    #         "dead": self.reward_scale * self.died_count * -0.5, #changed from -0.1 to -0.5
    #         "badge": self.reward_scale * self.get_badges() * 45, #changed from 10 to 45
    #         "explore": self.reward_scale * self.explore_weight * len(self.seen_coords) * 0.1,
    #         "stuck": self.reward_scale * self.get_current_coord_count_reward() * -0.05
    #     }
    #     # emphasize getting badges and not fainting?
    ...
    #     # Reward for new positions, penalty for revisits
    #     if position_key not in self.seen_coords:
    #         curiosity_reward = self.explore_weight * 0.5  # Reward for visiting a new position change from 0.2 to 0.5
    #         self.seen_coords[position_key] = 1  # Mark as visited in seen_coords
    #     else:
    #         curiosity_reward = -0.2  # Penalty for revisiting old position change from -0.05 to -0.2
